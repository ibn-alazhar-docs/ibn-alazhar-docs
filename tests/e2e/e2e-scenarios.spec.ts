...
test.describe("Ibn Al-Azhar Docs — Complete E2E Suite", () => {
  ...
  test("3. Upload PDF & Failed upload", async ({ page }) => {
    ...
    await page.locator('button', { hasText: /تأكيد|Confirm|رفع وبدء المعالجة|Upload & Process/ }).first().click({ timeout: 15000 });
  ...
  test("4. Search document & Empty search", async ({ page }) => {
    ...
    await expect(page.locator("text=لم نعثر على أية نتائج بحث تتطابق مع:").first().or(page.locator("text=لا يوجد").first())).toBeVisible({ timeout: 10000 });
  ...
  test("6. Create folder", async ({ page }) => {
    ...
    await page.getByRole("button", { name: "+ مجلد جديد" }).or(page.locator("text=+ مجلد جديد")).click({ force: true });
  ...
  test("7. Move document & 8. Add tags & 9. Export document", async ({ page }) => {
    ...
    const fileRow = page.locator('table tbody tr, [data-testid="file-list"] > div, .file-list > div').first();
  ...
  test("10. Create share link, 11. Open anonymously, 12. Expired share link", async ({ page, context }) => {
    await page.goto(`${BASE}/ar/login`);
    await page.fill('input[name="email"]', TEST_EMAIL);
    await page.fill('input[name="password"]', TEST_PASSWORD);
    await page.locator('button[type="submit"]').first().click({ force: true });
    ...
    if (shareLink) {
        ...
        await expect(anonymousPage.locator("text=مشاركة بواسطة").first().or(anonymousPage.locator("text=تحميل").first())).toBeVisible({ timeout: 15000 });
