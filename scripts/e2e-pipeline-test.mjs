...
async function main() {
  ...
  try {
    ...
    log(
      `Page sizes: ${splitResult.pages.map((p) => `${(p.length / 1024).toFixed(0)}KB`).join(", ")}`,
    );
    ...
    const ocrResult = await manager.extractPages(config, splitResult.pages, "لا_أعلم_هويتي.pdf");
