              ...
              <div className="mt-5 space-y-2">
                {recentDocuments.map((doc) => (
                  ...
                  >
                    ...
                    <svg className="h-4 w-4 shrink-0 text-very-muted transition-colors group-hover:text-[var(--btn-primary-bg)] rtl:rotate-180" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
function StatCard({
  ...
  const iconMap: Record<string, ReactNode> = {
    ...
    folders: (
      <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" /></svg>
    ),
    tags: (
      <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" /></svg>
    ),
    conversions: (
  ...
  return (
    <Link
      href={href}
      className="stat-card-premium rounded-2xl p-5 transition-all"
    >
      <div className="flex items-center justify-between">
        <Text color="muted" className="text-xs font-medium uppercase tracking-wide">
          {title}
        </Text>
        <span className="text-very-muted transition-colors" style={{ color: "var(--text-tertiary)" }}>
          {iconMap[icon]}
        </span>
      </div>
