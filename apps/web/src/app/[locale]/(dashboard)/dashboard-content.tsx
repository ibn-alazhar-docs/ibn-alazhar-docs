  ...
  return (
    <Container>
      <div className="py-8 space-y-8">
        {/* Page Header */}
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-primary-color">{tNav("home")}</h1>
          <p className="mt-1 text-sm text-secondary-color">{tDocs("uploadPrompt")}</p>
        </div>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              {recentDocuments.length > 0 && (
                <Link
                  href="/files"
                  className="text-xs font-medium text-success hover:underline transition-colors"
                >
                  {tDocs("viewAll")}
                </Link>
              )}
          ...
          <CardContent>
            {recentDocuments.length === 0 ? (
              <p className="text-sm text-muted-color py-6 text-center">{tDocs("empty")}</p>
            ) : (
                ...
                {recentDocuments.map((doc) => (
                  ...
                  >
                    <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-badge">
                      <FileTextIcon
                        className={`h-5 w-5 ${doc.mimeType === "application/pdf" ? "text-danger" : "text-info"}`}
                      />
                    </span>

                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm font-semibold text-primary-color transition-colors group-hover:text-success">
                        {doc.title}
                      </p>
                    ...
                    <ChevronRightIcon className="h-4 w-4 shrink-0 text-muted-color transition-colors group-hover:text-success rtl:rotate-180" />
                  </Link>
                ))}
              </div>
            )}
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-widest text-muted-color">
            {title}
          </p>
          <p
