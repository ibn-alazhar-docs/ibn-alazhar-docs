...
export const handlers = [
  ...
  http.put(`${API_BASE}/documents/:id`, async ({ request, params }) => {
    const body = (await request.json()) as Record<string, unknown>;
  ...
  http.get(`${API_BASE}/documents/:id/tags`, ({ params }) => {
    return HttpResponse.json({ documentId: params.id, tags: [] });
  }),

  http.post(`${API_BASE}/documents/:id/tags`, async ({ request, params }) => {
    const body = (await request.json()) as Record<string, unknown>;
