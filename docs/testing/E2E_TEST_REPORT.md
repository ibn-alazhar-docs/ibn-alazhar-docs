...
- _Observation_: The initial loading time (Turbopack compilation) on dev mode causes the very first Playwright run to time out after 120s if the server is not pre-warmed.
