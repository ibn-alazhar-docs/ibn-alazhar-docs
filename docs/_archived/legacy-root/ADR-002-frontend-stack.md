...
## السياق (Context)
منصة مستند / DocEd تحتاج إلى frontend framework يوفر: Server-Side Rendering (SSR) لتحسين أداء الصفحات الأولى وتحسين محركات البحث (SEO)، API routes مدمجة لبناء الـ backend داخل نفس المشروع، دعم PWA من خلال التكامل مع Service Workers، ونظام بيئي React قوي يسهل إيجاد مكتبات ومطورين. الاختيار هنا يؤثر على بنية المشروع بأكمله، سرعة التطوير، قابلية التوسع، وتجربة المطور (DX).
