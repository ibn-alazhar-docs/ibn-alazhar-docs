...
## السياق (Context)
منصة مستند / DocEd تحتاج إلى طبقة backend تعالج عدة أنواع من المهام: HTTP API endpoints للعمليات العادية (CRUD، مصادقة، استعلامات)، معالجة خلفية (background processing) للعمليات الثقيلة مثل OCR وتحويل الملفات، وتحديثات فورية (real-time updates) لإبلاغ المستخدم عن تقدم العمليات. الفريق صغير والمشروع في مرحلة MVP، لذا نحتاج إلى حل يوازن بين البساطة والقدرة على التوسع. المشاريع السابقة مثل Tahweel عانت من تعقيد البنية التحتية المنفصلة.
...
- تصميم قائمة الـ job types (OCR، convert، thumbnail، cleanup) وأولوياتها
