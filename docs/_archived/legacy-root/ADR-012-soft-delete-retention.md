...
## السياق (Context)
في منصة مستند / DocEd، يقوم المستخدمون بإدارة مستندات تعليمية مهمة (أوراق اختبار، واجبات، ملاحظات). حذف ملف بالخطأ يمكن أن يكون كارثيًا، خاصة إذا كان الملف الوحيد الذي يملكه المستخدم. في مشروع Tahweel السابق، كان الحذف نهائيًا (hard delete)، مما أدى إلى طلبات متكررة من المستخدمين لاستعادة ملفات محذوفة كان لا يمكن تلبيتها. نحتاج إلى آلية تتيح استعادة الملفات المحذوفة دون الاحتفاظ بها إلى الأبد (مما يستهلك مساحة تخزين). التوازن المطلوب هو بين إمكانية الاستعادة والكفاءة في استخدام التخزين.
...
- بعد 30 يومًا، يُحذف الملف نهائيًا عبر cron job (hard delete + حذف من MinIO)
...
## المتابعة (Follow-up)
- إضافة حقل `deletedAt` لكل models ذات الصلة في Prisma schema
...
- اختبار عملية الاستعادة والحذف النهائي
